...
import { getSearchHistories } from 'apiSdk/search-histories';
...
import { FoodItemListPage as ShowCaseList } from 'pages/food-items';

function DashboardPage() {
  const { session } = useSession();

  const searchHistoryStat = useSWR<PaginatedInterface<SearchHistoryInterface>>(
    () => `/search-histories`,
    () => getSearchHistories({ limit: 1, offset: 0 }),
  );
  ...
  const foodItemStat = useSWR<PaginatedInterface<FoodItemInterface>>(
    () => `/food-items`,
    () => getFoodItems({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Search Histories',
      result: searchHistoryStat?.data?.totalCount,
      isLoading: searchHistoryStat.isLoading,
    ...
    {
      title: 'Food Items',
      result: foodItemStat?.data?.totalCount,
      isLoading: foodItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`food-items`} passHref legacyBehavior>
                  <Button
