...
type ColumnType = ColumnDef<FoodItemInterface, unknown>;

interface FoodItemListPageProps {
...
export function FoodItemListPage(props: FoodItemListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getFoodItems({
        relations: ['vendor', 'order_item.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FoodItemInterface>>(
    () => `/food-items?params=${JSON.stringify(params)}`,
    fetcher,
  );
    ...
    hasAccess('vendor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/vendors/view/${record.vendor?.id}`}>
              {record.vendor?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/food-items/view/${record.id}`} passHref legacyBehavior>
                <Button
