...
type ColumnType = ColumnDef<OrderItemInterface, unknown>;

interface OrderItemListPageProps {
...
export function OrderItemListPage(props: OrderItemListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<OrderItemInterface>>(
    () => `/order-items?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: OrderItemInterface) => {
    if (hasAccess('order_item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/order-items/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'quantity', header: 'quantity', accessorKey: 'quantity' },
    hasAccess('food_item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'food_item',
          header: 'Food Item',
          accessorKey: 'food_item',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/food-items/view/${record.food_item?.id}`}>
              {record.food_item?.name}
            </Link>
          ),
        }
      : null,
