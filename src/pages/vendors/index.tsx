...
type ColumnType = ColumnDef<VendorInterface, unknown>;
...
export function VendorListPage(props: VendorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getVendors({
        relations: ['user', 'food_item.count', 'order.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<VendorInterface>>(
    () => `/vendors?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: VendorInterface) => {
    if (hasAccess('vendor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/vendors/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('food_item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.food_item,
        }
      : null,
