...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order.create': {
    ...
    tenantPath: ['vendor', 'order'],
    userPath: [],
  },
  'order.update': { roles: ['customer'], key: 'order-status-update', tenantPath: ['vendor', 'order'], userPath: [] },
  'food_item.update': {
    roles: ['business-owner', 'manager'],
    key: 'food-item-update',
    tenantPath: ['vendor', 'food_item'],
    userPath: [],
  },
};
...
const tenantRoles: string[] = ['business-owner', 'manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.vendor.findFirst({
